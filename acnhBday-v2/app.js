//#ChatGPT Log https://chat.openai.com/share/2427cde2-f571-45d7-9a8b-7c9a202c3e54
...
window.addEventListener('load', () => {
    ...
    fetch("https://api.nookipedia.com/villagers?api_key=b3f79a4d-9149-49a6-8d09-606528b847a4")
        .then(function (reImage) {
            console.log(reImage);
            return reImage.json();
        })
        .then(data => {
            ...
            for (const key in data) {
                if (data.hasOwnProperty(key)) {
                    ...
                    nookName = element['name'];
                    villagerImage = element['image_url'];
    ...
    submitButton.addEventListener('click', () => {
        ...
        for (const villagerKey in acnhData) { //acnhData already defined as data
            if (acnhData.hasOwnProperty(villagerKey)) {
                ...
                ////////////////////////////
                // MATCH BIRTHDAY TO DATA //
                ////////////////////////////

                //1. Match Bday to Input Date
                if (villagerBday === inputDate) {

                    const matchingVillager = imgData.find(villagerImg => villagerImg.name === name); //pulling data from imgData API
                    // console.log(matchingVillager);

                    //2. Check if Elements have been created
                    let intro = document.querySelector(".intro");
                    let description = document.querySelector(".description");
                    let headshot = document.querySelector(".headshot")

                    //3. Match names from both datasets
                    if (matchingVillager) {
                        const villagerImageURL = matchingVillager.image_url;

                        //4. Check if elements already created
                        if (!intro) {

                            //a. Create Intro Element + append to container
                            ...
                            //c. Create Headshot Element + append to container
                            headshot = document.createElement("img");
                            headshot.src = villagerImageURL; //to be replaced with json data
                            headshot.setAttribute("class", "headshot"); //class="headshot"
                            villagerData.appendChild(headshot);

                            //d. Create Description Element + append to container
                                //Info about villager is created in villagerData (already defined)
                        ...
                        ${pronoun} favorite saying is '<span class="highlight">${saying}</span>'`; //add json data - gender, personality, species, hobby, saying
                            ...
                            //e. Replace data if HTML elements already exist
...
function draw() {
    ...
    //Fall  
        ...
        for (var i = 0; i < random(1); i++) {
            if (random(3) < 0.5) { //lower probability, so more scattered leaves
        ...
        for (let blade of leaves) {
            blade.update(t); //update leaf position
